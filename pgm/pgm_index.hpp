...
/**
 * note_by_ywm:pgm_index实际就提供了两个功能性的函数，
 * build提供构造索引的接口，实际上是递归构建每一层的段序列直到==1,
 * 实际底层依赖的是make_segmentation函数；
 * search 提供搜索接口，调用的是segment_for_key函数
 */
#pragma once
...
#define PGM_ADD_EPS(x, epsilon, size) ((x) + (epsilon) + 2 >= (size) ? (size) : (x) + (epsilon) + 2)
    ...
    protected:
                          ...
                          std::vector<size_t> &levels_offsets)
        {
            ...
            last -= ignore_last;
            // call make_segmentation function to build pgm index
            ...
            {
                auto x = first[i];
                // 这里的对重复值处理的逻辑有些微问题,可以考虑修改一下看看，或者不处理是不是也没关系呢
...
#define BLOCK_RECORDING 1
        ...
        void read_block_or_not(FILE *file_handler, int32_t *last_block, int32_t block_to_fetch, char *block_data)
        {
            ...
            {
                // std::cout<<"block_to_fetch != *last_block"<<std::endl;
    ...
    public:
        ...
        /**
         * Constructs the [in-memory] index on the sorted keys in the range [first, last).
