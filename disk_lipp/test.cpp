...
void test_lipp_insert(char *index_name, char *key_path, int count, int has_size, int insert_count=100) {
    StorageManager<KeyType, ValueType> index;
    index.init(index_name, true);
    std::ifstream fin(key_path, std::ios::binary);
    if(!fin.is_open()){
        std::cerr<<"this file is cannot open,path:"<<key_path<<std::endl;
    }
    KeyType *keys = new KeyType[count];
    if (has_size == 1) {
        uint64_t size;
