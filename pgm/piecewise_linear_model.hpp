...
/**
 * note_by_ywm:kernel function就是make_segmentation 和add_points
 * make_segmentation用于向pgm_index提供一个在数据集上构建段序列的接口
 * add_points是具体的凸包流算法实现,用于生成一个segment
 */
...
namespace pgm::internal
{
    ...
    public:
        ...
        /**
         * 用于向模型中添加一个点
         * 它的主要任务是根据给定的误差阈值 (epsilon)，维护一个凸包以支持分段线性模型的构建
         */
        bool add_point(const X &x, const Y &y)
        {
            //check x is increasing
            ...
            //检查点是否在当前凸包内
            auto slope1 = rectangle[2] - rectangle[0];//min_2-max_1=(y2-y1-2eps)/(x2-x1)
            auto slope2 = rectangle[3] - rectangle[1];//max_2-min_1=(y2-y1+2eps)/(x2-x1)
    ...
    class OptimalPiecewiseLinearModel<X, Y>::CanonicalSegment
    {
        ...
        {
            return rectangle[0].x == rectangle[2].x && rectangle[0].y == rectangle[2].y && rectangle[1].x == rectangle[3].x && rectangle[1].y == rectangle[3].y;
        }
    ...
    /**
     * @param n:数组A的大小，epsilon 误差阈值参数,
     * @param Fin in 输入读取函数，in(i) returns pair{key,pos}
     */
    ...
    size_t make_segmentation(size_t n, size_t epsilon, Fin in, Fout out)
    {
        ...
        OptimalPiecewiseLinearModel<X, Y> opt(epsilon); // create PLA model
        opt.add_point(p.first, p.second);               // add first point
...
#pragma omp parallel for reduction(+ : c) num_threads(parallelism)
    ...
    auto make_segmentation(RandomIt first, RandomIt last, size_t epsilon)
    {
        ...
        auto in_fun = [first](auto i)
        { return pair_type(first[i], i); };
        auto out_fun = [&out](const auto &cs)
        { out.push_back(cs); };
