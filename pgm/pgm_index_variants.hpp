...
#include <immintrin.h>
    ...
    public:
        ...
        /**
         * [in-memory] Constructs the multidimensional container with the elements in the range [first, last).
        ...
        /**
         * [on-disk] Constructs the multidimensional container with the elements in the range [first, last).
         * @param first, last the range to copy the elements from
         */
        template <typename RandomIt>
        MultidimensionalPGMIndex(RandomIt first, RandomIt last, bool _inner_disk, FILE *file_handler) : data(), pgm()
        {
            data.reserve(std::distance(first, last));
            std::for_each(first, last, [&](const auto &x)
                          {
            ...
            data.emplace_back(encode(x)); });
            std::sort(data.begin(), data.end());
            pgm = decltype(pgm)(data.begin(), data.end(), _inner_disk, file_handler);
        }
        // // load multi_pgm index from disk file
        // load_disk(FILE* file_handler,bool inner_disk)
        // {
        //     pgm=decltype(pgm)(file_handler,inner_disk);
        // }
        ...
        std::vector<value_type> range_query(FILE* file_handler,const value_type &min, const value_type &max,int &tc){
            ...
            tc+=c;
            //traverse min_pos and max_pos And find the result
            auto min_it=std::lower_bound(data.begin()+min_pos.lo,data.begin()+min_pos.hi,zmin);
            auto max_it=std::upper_bound(data.begin()+max_pos.lo,data.begin()+max_pos.hi,zmax);
            //add data block access
            tc+=((max_it-min_it)/341);
            for(auto it=min_it;*it<zmax&&it!=max_it;it++)
            {
                if(box_zcontains(zmin,zmax,*it))
                {
                    value_type p=morton::Decode(*it);
        ...
        // todo:on-disk version
        iterator range(const value_type &min, const value_type &max, FILE *file, int *c, int *lc) { return iterator(this, min, max, file, c, lc); }
        /**
    ...
    private:
        ...
        public:
            ...
            RangeIterator(const decltype(super) super, const value_type &min, const value_type &max)
                : super(super),
                  ...
                  miss(0)
            {
                // std::cout<<"call range"<<std::endl;
                if (zmin > zmax)
                    throw std::invalid_argument("min > max");

                auto range = super->pgm.search(zmin);
                //find the precise position of zmin in 2*eps bound
            ...
            // disk range iterator
            RangeIterator(const decltype(super) super, const value_type &min, const value_type &max, FILE *file_handler, int *c, int *lc)
                : super(super),
                  min(min),
                  max(max),
                  zmin(multidimensional_pgm_type::encode(min)),
                  ...
                  miss(0)
            {
                std::cout<<"enter rangeIter disk"<<std::endl;
                if (zmin > zmax)
                    throw std::invalid_argument("min > max");
                // first get min_corner beg position,type:ApproxPos
                auto beg = super->pgm.search_disk(file_handler, zmin, c, lc);
                // this->it = std::lower_bound(super->data.begin() + range.lo, super->data.begin() + range.hi, zmin);
                // if (this->it == super->data.end())
                //     return;
        ...
        {
            return apply([](const auto &head, const auto &...tail)
                         { return morton::Encode(head, tail...); }, t);
        }
        ...
        {
            //通过判断p每个维度的值是否在min和max之间判断p是否在范围内
            return (((min & (selector << I)) <= (p & (selector << I)) && (p & (selector << I)) <= (max & (selector << I))) && ...);
        }
